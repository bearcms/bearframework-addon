    ...
    <head><?php
        if ($renderElementsContainer) {
            ...
            if ($hasLazyLoading) {
                ...
                echo '<script id="bearcms-bearframework-addon-script-3" src="' . htmlentities($context->assets->getUrl('assets/elementsLazyLoad.js', ['cacheMaxAge' => 999999999, 'version' => 2])) . '" async onload="' . htmlentities('bearCMS.elementsLazyLoad.initialize(' . json_encode($lazyLoadInitializeData) . ');') . '"></script>';
