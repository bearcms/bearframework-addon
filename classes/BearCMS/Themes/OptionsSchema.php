        ...
        $walkOptions = function(&$options) use (&$walkOptions, &$valuesSetCount, $valuesCount, $values) {
            foreach ($options as $i => $option) {
                if ($option instanceof \BearCMS\Themes\OptionsGroupSchema) {
                    ...
                    $options[$i]['value'] = $values[$option['id']];
                    $valuesSetCount++;
        ...
        $walkOptions = function($options) use (&$cssRules, &$cssCode, &$walkOptions) {
            foreach ($options as $option) {
                if (is_array($option) && isset($option['id'])) {
                    $value = isset($option['value']) ? (is_array($option['value']) ? json_encode($option['value']) : $option['value']) : null;
                ...
                } elseif ($option instanceof \BearCMS\Themes\OptionsGroupSchema) {
